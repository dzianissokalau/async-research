...
| TBD | P2: README rewrite | Planned | Rewrite the root README for first-time users. | Add a plain-English research loop, lifecycle diagram, worked task loop, command read/write map, and explicit Python 3.11+ setup guidance. | Not started. |
| TBD | P2: Exit codes and CLI behavior | Planned | Document exit codes and improve CLI help. | Centralize or document command-specific exit codes, add useful help text for top-level and nested subcommands, and preserve existing command names. | Not started. |
| TBD | P2: Script import cleanup | Planned | Refactor sibling-script imports without behavior changes. | Replace `sys.path.insert` patterns with package imports while preserving CLI and `python -m` execution paths. | Not started. |
| TBD | P2: Benchmark and simulation packaging | Planned | Make benchmark and simulation behavior more robust from installed packages. | Remove repo-root layout assumptions, prefer in-process module invocation where practical, and keep workspace isolation guards based on explicit user paths. | Not started. |
| TBD | P3: Package metadata and repo hygiene | Planned | Polish release metadata and contributor affordances. | Add project URLs, keywords, explicit license metadata, changelog, contributor guidance, and issue/PR templates when external contribution starts. | Not started. |
| TBD | P3: Optional command naming cleanup | Planned | Consider clearer aliases without breaking existing commands. | Prefer additive aliases and docs updates before any command rename, and preserve alpha users' current workflows. | Not started. |
| TBD | P3: Optional CLI architecture refactor | Planned | Defer splitting `cli.py` until there is measured command-count or review pain. | If justified later, split internals while preserving command behavior and JSON output contracts. | Not started. |
| TBD | P3: Optional docs packaging review | Planned | Revisit packaged protocol docs only after the public/private docs split is stable. | Keep useful operator and agent docs packaged for now, then measure wheel size and audience needs before changing package data. | Not started. |
