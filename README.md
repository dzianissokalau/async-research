...
| `async-research idea capture research_ops --from-inbox row-7 --id IDEA-0007 --dry-run` | Preview explicit discovery-to-catalog capture. | `discovery_inbox.md`, canonical catalog records, duplicate checks. | JSON proposal only; Phase 6 refuses `--write`. |
| `async-research idea catalog maintain research_ops --dry-run` | Preview catalog maintenance proposals. | `discovery_inbox.md`, canonical catalog records, accepted output and rejected idea refs. | JSON proposal only; never edits `queue.md`, task folders, or canonical JSON in Phase 6. |
...
| `idea capture` and `idea catalog maintain` | `0` dry-run proposal printed. | `3` invalid capture input or refused Phase 6 write mode. |
