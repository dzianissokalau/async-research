...
| `async-research console research_ops` | Serve the local dashboard shell on `127.0.0.1:8765`. | Packaged console assets, the read-only snapshot API, setup action catalog, and guarded setup/health actions. | Serves static assets, `GET /api/snapshot`, `GET /api/actions`, and `POST /api/actions/run`; dashboard mutations are limited to explicit `init` and `surface update` actions. |
