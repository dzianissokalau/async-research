...
| `async-research idea promote research_ops IDEA-0001 --write --preflight-hash <hash>` | Write one planner-facing promotion proposal reference after a matching dry run. | The dry-run preflight hash, selected canonical idea JSON record, catalog lock, and reserved task-id preflight. | Reserves a deterministic task id, appends `inbox.md`, updates selected idea proposal refs and generated projections; never edits `queue.md` or creates task folders in V2.5. |
