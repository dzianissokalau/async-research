    ...
    cli.write_text(
        "\n".join(
            [
                ...
                "fi",
                "if [ \"$1\" = \"--help\" ]; then",
                "  printf 'usage: async-research {version,init,schema-check,mode,readiness,health,workflow,console}\\n'",
                "  exit 0",
                "fi",
                "if [ \"$1\" = \"mode\" ] && [ \"$2\" = \"--help\" ]; then",
                "  printf 'usage: async-research mode {show,validate}\\n'",
                "  exit 0",
                "fi",
                ...
                "  exit 0",
                "fi",
                "if [ \"$1\" = \"mode\" ] && [ \"$2\" = \"show\" ]; then",
                "  printf '{\"ok\": true, \"mode\": \"manual\"}\\n'",
                "  exit 0",
                "fi",
                "if [ \"$1\" = \"mode\" ] && [ \"$2\" = \"validate\" ]; then",
                "  printf '{\"ok\": true, \"check\": \"mode\"}\\n'",
                "  exit 0",
                "fi",
                "if [ \"$1\" = \"readiness\" ]; then",
