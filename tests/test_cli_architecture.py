...
class CliArchitectureTests(unittest.TestCase):
    ...
    def test_workflow_commands_route_to_public_orchestrator(self) -> None:
        ...
        with mock.patch.object(cli, "module_main", return_value=cli.SUCCESS) as module_main:
            code = cli.main(["workflow", "next", "research_ops", "--stale-minutes", "5"])

        self.assertEqual(cli.SUCCESS, code)
        module_main.assert_called_once_with(
            "workflow_orchestrator",
            ["next", "research_ops", "--stale-minutes", "5.0"],
        )
    ...
    def test_build_parser_registers_nested_aliases(self) -> None:
        ...
        self.assertEqual(["append", "check", "resolve-task", "auto-resolve-task", "summarize"], list(decision_choices))
        ...
        self.assertEqual(["validate", "dashboard", "inspect-proposals", "apply-proposals"], list(data_choices))
        self.assertEqual(["init", "validate", "dashboard", "inspect-proposals", "apply-proposals"], list(library_choices))
        ...
        self.assertEqual(["score", "validate", "capture", "promote", "metrics", "trace", "resolve", "park", "reject", "catalog"], list(idea_choices))
