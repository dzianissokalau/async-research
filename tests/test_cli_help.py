...
class CliHelpTests(unittest.TestCase):
    ...
    def test_every_public_command_help_has_operator_context(self) -> None:
        cases = [
            ...
            (["mode"], ["interaction mode", "show", "set", "validate", "manual-compatible default"]),
            (["mode", "show"], ["effective interaction mode", "missing-config default", "Exits 0", "invalid or unreadable"]),
            (["mode", "set"], ["interaction_mode.json", "--mode", "conservative defaults", "existing config is invalid"]),
            (["mode", "validate"], ["schema", "hard-stop safeguards", "without mutating research_ops", "fails closed"]),
            ...
            (["decision"], ["decisions.md", "append", "check", "resolve-task", "auto-resolve-task", "summarize"]),
            ...
            (["decision", "auto-resolve-task"], ["framework-policy", "interaction mode", "--dry-run"]),
            ...
            (["runtime"], ["runtime evidence", "trace ledgers", "validate", "summary", "inspect-evidence", "dry-run", "execute"]),
            (["runtime", "validate"], ["evidence_objects.jsonl", "traces.jsonl", "snapshot hashes", "permission metadata", "Exits 0"]),
            (["runtime", "summary"], ["runtime trace count", "evidence object count", "unsupported or stale evidence count", "without mutating"]),
            (["runtime", "inspect-evidence"], ["EVID-*", "related traces", "without treating it as accepted evidence"]),
            (["runtime", "dry-run"], ["runtime request JSON", "task contract", "planned local or mocked adapter calls", "without writing"]),
            (["runtime", "execute"], ["local adapters", "mock external adapters", "research_ops/runtime", "without changing task state"]),
            (["eval"], ["trace-driven runtime eval", "build-from-traces", "run", "compare"]),
            (["eval", "build-from-traces"], ["runtime traces", "claim verification", "Without --write", "research_ops/evals"]),
            (["eval", "run"], ["deterministic graders", "without network", "cost/latency", "Exits 0"]),
            (["eval", "compare"], ["candidate eval metrics", "baseline", "release-policy blockers", "Exits 0"]),
            (["evidence-memory"], ["structured accepted-evidence memory", "claim verification", "deliverable links", "targeted reflections"]),
            (["evidence-memory", "update"], ["structured evidence memory index", "claim ids", "evidence ids", "contradiction edges", "--dry-run"]),
            (["evidence-memory", "query"], ["stale or contradicted evidence", "--contradictions-only", "--failure-class", "--reflection-threshold"]),
            (["scaling"], ["file-backed scaling friction", "dashboard snapshot latency", "optional rebuildable index cache", "assess"]),
            (["scaling", "assess"], ["task-local locks", "read-only console snapshot", "never moves truth out of research_ops", "--max-task-statuses", "--skip-dashboard-latency"]),
            (["brief"], ["research_brief.json", "output target", "audience", "source policy", "permissions", "human gates"]),
            (["brief", "draft"], ["research_ops/briefs/source_request.md", "--question", "--output-maturity", "--audience", "--write"]),
            (["brief", "validate"], ["output target", "audience", "unresolved questions", "human gates", "Exits 0"]),
            (["brief", "apply"], ["dry-run", "validated brief", "does not mutate research_ops", "--dry-run"]),
            ...
            (["reflection"], ["targeted reflection", "failure-class", "anti-context"]),
            (["reflection", "record"], ["targeted_reflection_v1.0", "--failure-class", "--review-evidence", "--dry-run"]),
            ...
            (["analysis"], ["Preflight", "dashboard", "reviewer-packet", "run-adapter", "preflight", "validate-run", "validate-results"]),
            (["analysis", "dashboard"], ["Read-only dashboard", "active run_analysis", "accepted empirical evidence", "--max-items", "--now"]),
            (["analysis", "reviewer-packet"], ["Read-only reviewer packet", "accepted experiment plan", "validator outputs", "without accepting evidence"]),
            ...
            (["idea"], ["idea-evaluation JSON artifacts", "score", "validate", "capture", "promote", "metrics", "trace", "resolve", "park", "reject", "catalog"]),
            ...
            (["idea", "metrics"], ["lifecycle metrics", "accepted outputs", "cost ledger", "--now", "without mutating"]),
            (["idea", "trace"], ["promoted task exists", "promotion trace fields", "accepted-output", "--now"]),
    ...
    def test_readme_documents_exit_code_contract(self) -> None:
        ...
        for snippet in [
            ...
            "| `idea metrics` and `idea trace` | `0` read-only lifecycle report printed.",
            ...
            "| `analysis reviewer-packet` | `0` packet context is complete.",
            ...
            "| `deliverable init`, `deliverable target`, and `deliverable response` | `0` manifest write succeeded.",
