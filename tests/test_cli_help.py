...
class CliHelpTests(unittest.TestCase):
    ...
    def test_every_public_command_help_has_operator_context(self) -> None:
        cases = [
            ...
            (["console"], ["Serve the local read-only dashboard shell", "console snapshot research_ops --json", "GET /api/snapshot", "--host", "--port", "--json", "--now"]),
            (["console", "snapshot"], ["Serve the local read-only dashboard shell", "read-only JSON snapshot", "GET /api/snapshot", "--json", "--now"]),
