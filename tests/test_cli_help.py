...
class CliHelpTests(unittest.TestCase):
    ...
    def test_every_public_command_help_has_operator_context(self) -> None:
        cases = [
            ...
            (["source"], ["data_source_audit.md", "validate", "freshness"]),
            (["source", "validate"], ["source audit rows"]),
            (["source", "freshness"], ["freshness windows"]),
            (["cost"], ["cost_ledger.csv", "summary"]),
            (["cost", "summary"], ["aggregate spend"]),
            (["metrics"], ["metrics_history.jsonl", "append"]),
            (["metrics", "append"], ["--label", "--update-weekly-digest"]),
            (["accepted"], ["accepted_outputs_index.md", "revalidation", "revalidate"]),
