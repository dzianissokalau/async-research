...
| 6 | Dashboard surface | Complete | Add read-only views for approved, candidate, blocked/restricted/deprecated, stale, gap-blocked, and join-caveat data states. | Adds `async-research data dashboard research_ops`, a JSON read-only dashboard over validator/read-model output with usable-today sources, candidate/blocked/stale groups, data gaps, data-blocked ideas, join paths/caveats, and no file mutation. |
