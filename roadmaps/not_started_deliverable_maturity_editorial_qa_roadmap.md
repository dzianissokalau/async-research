...
| P0 | 4 | Honest final-output naming | Rename or constrain labels like `final paper production` unless deliverable gates prove external readiness. Prefer `internal draft assembly` for TASK-0015-style work. | Removes the most misleading product signal from the coffee pilot. | Complete |
...
| P1 | 4 | Dashboard maturity and editorial QA panel | Show maturity target/current level, checklist completion, critic result, response matrix status, unresolved gaps, caveats, confidence, and review independence. | Lets humans trust the deliverable status without opening raw review files. | Complete |
