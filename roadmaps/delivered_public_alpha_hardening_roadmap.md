...
| 2026-05-06 | CLI audit wrapper-drift follow-up | Done | Closed the two remaining direct-script doc snippets for helpers that already have public wrappers. | Exposed `acceptance-suite --keep-work-dir` and health budget override flags through `async-research`, migrated the acceptance and health docs to public commands, and added regression guards against direct `run_acceptance_suite` and `health_check` invocations. | This change; CLI architecture/help/doc-reference tests, acceptance-suite, health budget smoke, full unittest discovery, acceptance-suite, benchmark, starter-smoke, and compileall passed. |
