...
10. Treat the promotion dry-run as authoritative. Inspect `evidence_support.status`: `thin_evidence` should usually stay `literature_extract`, while `missing_library_support` means unresolved `library_refs` need resolved `LIT-*` support or an earlier extraction task before library-dependent routes. If action is idea_promotion_blocked, do not write a task; list the blockers and required human decisions. If action is idea_promotion_planned, keep promotion_preflight_hash and the proposed TASK ID/slug.
