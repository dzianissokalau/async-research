...
24. Use `mode: "parallel_research"` only for source gathering, literature extraction, market-map slices, jurisdiction comparisons, or data-source profiling when the task contract explicitly enables `runtime_permissions.parallel_research`. A parallel request must name branch-specific allowed paths, branch budgets, a task-lock coordinator, and a deterministic merge packet under `research_ops/runtime/parallel_merges/`; it cannot skip claim verification, review, result acceptance, or human gates.
25. For each written task, run async-research anti-context build research_ops --title "<candidate title>" --task-dir <task-dir> before assigning a worker when anti-context is required for the task class.
26. If anti-context shows similar accepted findings, rejected approaches, or stale accepted memory, revise or pause the written task through the normal task revision/human decision flow rather than editing queue.md by hand.
27. Update daily_status.md.
