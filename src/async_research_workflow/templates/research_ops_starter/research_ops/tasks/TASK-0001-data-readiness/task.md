...
Verify whether the starter real-estate data sources in `research_ops/data_source_audit.md`
are usable for the first automated research loop.

## Scope

- Work only inside this task folder and `research_ops/data_source_audit.md`.
...
- Each source is marked `available`, `usable_with_caveats`, `blocked`,
  `restricted`, or left as `candidate` with a reason.
...
- `async_research_workflow/examples/scripts/data_source_audit.py`

## Data Source Audit

This is a `data_readiness` task. It may update `DS-0001`, `DS-0002`, and
`DS-0003`, but it should not create an experiment plan.
